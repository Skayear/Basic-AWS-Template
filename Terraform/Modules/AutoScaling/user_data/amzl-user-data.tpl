...
for i in {1..5}
do
  yum install -y aws-cli awslogs wget jq && break || sleep 60
done
...
# Use jq to translate the JSON file into the CloudWatch agent configuration format
echo '{
	"agent": {
...
sudo /opt/aws/amazon-cloudwatch-agent/bin/amazon-cloudwatch-agent-ctl -a fetch-config -m ec2 -s -c file:/opt/aws/amazon-cloudwatch-agent/etc/cloudwatch-agent-config.json

sudo /opt/aws/amazon-cloudwatch-agent/bin/amazon-cloudwatch-agent-ctl -m ec2 -a start

#END INSTALL FOR CLOUDWATCH AGENT
